  ...
  return (
    ...
    >
      <div className="fixed top-0 left-0 right-0 p-4 flex justify-between items-center z-10">
        ...
        <button className="w-[50px] text-right font-semibold text-[17px] leading-[22px] tracking-[-0.41px] text-[#667CFF] transition-all duration-200 hover:opacity-50 active:opacity-50" onClick={handleCloseButton}>
          닫기
        </button>
      </div>
      <div className="flex-grow overflow-y-auto p-4 pt-[60px] pb-[60px]">
        <div className="relative mt-[32px]">
          <img src="/card1.png" alt="Card 1" className="w-[206px] h-[321px] absolute top-0 left-7" style={{ transform: 'rotate(-12.002deg)', aspectRatio: '43/67' }} />
          <img src="/card2.png" alt="Card 2" className="w-[197.483px] h-[307.502px] absolute top-[86px] right-7" style={{ transform: 'rotate(12deg)', aspectRatio: '70/109' }} />
        </div>
        <div className="mt-[460px] text-[16px] font-regular leading-[22px] tracking-[-0.02em] text-[#AEAEAE]">
          암호화폐를 일상 결제로 연결하는 방법, <br />KaiaPay 카드가 곧 여러분을 찾아갑니다
        </div>
        <div className="h-6" />
        <div className="text-[32px] font-bold leading-[44px] tracking-[-0.03em] text-white">
          애플페이, 삼성페이<br />어디서든 크립토로 결제
        </div>
        <div className="h-6" />
        <div className="flex flex-col gap-8 p-4 pt-6 pb-4 rounded-lg bg-white/10">
          {
            [
              {
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 z-10">
        <Button backgroundColor="bg-secondary" textColor="text-white" className="border border-white/20">카드 미리 신청하기</Button>
      </div>
      <BottomSheet
