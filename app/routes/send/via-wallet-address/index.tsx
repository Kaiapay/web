...
import AnnotationAlertIcon from "~/components/icons/AnnotationAlertIcon";
...
export default function SendViaWalletAddress() {
  ...
  const onSubmit: SubmitHandler<IFormInput> = (data) => {
    ...
    if (!regex.test(data.walletAddress)) {
      setError('walletAddress', { type: 'manual', message: '올바른 지갑 주소가 아닙니다.' });
      setIsLoading(false);
      return;
    }
    setIsLoading(true);
  ...
  return (
    ...
    <div 
      ...
      {errors.walletAddress ? <XCircleIcon size={16} /> : <CheckSmallIcon />}
        <div className="font-normal text-[12px] leading-[14px] tracking-[-0.1px]" style={{ color: errors.walletAddress ? '#FF443D' : 'rgba(255, 255, 255, 0.5)' }}>
          {errors.walletAddress ? errors.walletAddress.message : '카이아 네트워크로만 보낼 수 있어요'}
        </div>
      </div>
