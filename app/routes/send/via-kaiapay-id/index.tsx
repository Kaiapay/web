...
export default function SendViaKaiapayId() {
  ...
  const onSubmit: SubmitHandler<IFormInput> = async (submitData) => {
    ...
    try {
      ...
      const { publicAddress, transactionId } = result;
      try {
        await transferToken({
          transactionId,
      ...
      <form>
        ...
        >
          <HeaderWithBackButton
            heading="카이아페이 아이디"
            subheading="받는 사람의 KaiaPay 아이디를 입력해주세요"
          />
          <div className="h-8" />
          ...
          <div className="flex flex-row items-center gap-2 pb-1 px-8">
            ...
            <div
              ...
              style={{
                color: errors.kaiapayId
                  ? "#FF443D"
                  : "rgba(255, 255, 255, 0.5)",
              }}
            >
              {errors.kaiapayId
                ? errors.kaiapayId.message
                : "한글, 영문, 숫자 4-16자"}
            </div>
          </div>
